...
function initHeroSlider() {
  ...
  let timer = null;
  const PHOTO_MS = 5000;
  const VIDEO_MAX_MS = 12000; // cap long videos so the hero keeps moving
  ...
  function slideDuration(slide) {
    const v = slide.querySelector('video');
    if (!v) return PHOTO_MS;
    // Prefer real duration when available, else max
    const d = (v.duration && isFinite(v.duration) ? v.duration * 1000 : VIDEO_MAX_MS);
    return Math.min(Math.max(d, 4000), VIDEO_MAX_MS);
  }
